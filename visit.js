...
export default async function handler(req, res) {
  ...
  try {
    // GET: получение записей счетчика за сегодня
    if (req.method === 'GET') {
    ...
    // POST: фиксация визита (запись IP в таблицу site_visits и обновление счетчика)
    if (req.method === 'POST') {
      ...
      // 2. Добавляем запись с IP в таблицу истории визитов
      ...
      // 3. Логика счетчика: проверяем, есть ли запись за сегодня в таблице unique_daily_visits
      ...
      if (existingRecord) {
        // Если запись есть — инкрементируем счетчик на 1
